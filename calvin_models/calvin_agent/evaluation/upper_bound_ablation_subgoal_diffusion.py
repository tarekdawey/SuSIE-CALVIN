...
class CustomModel(CalvinBaseModel):
    def __init__(self):
        ...
        self.log_dir = "/nfs/kun2/users/pranav/calvin-sim/experiments/diffusion_policy_experiments/oracle_gcbc_subgoal_diffusion/task" + str(TASK_ID)
...
def evaluate_policy(model, env, epoch=0, eval_log_dir=None, debug=False, create_plan_tsne=False):
    ...
    for i in range(25):
        ...
        success = rollout(env, model, task_oracle, task, val_annotations, plans, False)
        if success:
            num_successes += 1
    print("###################")
